...
const DefaultTopN = 3;
const DefaultNoAnswer = "I am sorry, I don't understand.";
...
class QnAMakerBaseDialog extends ComponentDialog {
  ...
  constructor(qnaService) {
    ...
    this.addDialog(
      new WaterfallDialog(QNAMAKER_DIALOG, [
  ...
  async callGenerateAnswerAsync(stepContext) {
    ...
    if (dialogOptions[QnAOptions] != null) {
      ...
      qnaMakerOptions.scoreThreshold = qnaMakerOptions.scoreThreshold
        ? qnaMakerOptions.scoreThreshold
        : DefaultThreshold;
      qnaMakerOptions.top = qnaMakerOptions.top
        ? qnaMakerOptions.top
        : DefaultThreshold;
    }
    ...
    if (previousContextData != null && prevQnAId != null) {
      if (prevQnAId > 0) {
        ...
        if (
          previousContextData[
          stepContext.context.activity.text.toLowerCase()
          ] !== null
        ) {
          qnaMakerOptions.qnaId =
            previousContextData[
            stepContext.context.activity.text.toLowerCase()
            ];
        }
    ...
    var response = await this._qnaMakerService.getAnswersRaw(
      stepContext.context,
      qnaMakerOptions
    );
    ...
    if (
      ...
      response.answers[0].score <= 0.95
    ) {
      response.answers = this._qnaMakerService.getLowScoreVariation(
        response.answers
      );
      ...
      if (response.answers.length > 1) {
        ...
        var message = QnACardBuilder.GetSuggestionCard(
          suggestedQuestions,
          qnaDialogResponseOptions.activeLearningCardTitle,
    ...
    if (trainResponses.length > 1) {
      ...
      if (qnaResults.length > 0) {
        ...
        return await stepContext.replaceDialog(
          QNAMAKER_DIALOG,
          stepContext.activeDialog.state.options
        );
      }
    }
  ...
  async checkForMultiTurnPrompt(stepContext) {
    if (stepContext.result != null && stepContext.result.length > 0) {
      ...
      if (
        answer.context != null &&
        answer.context.prompts != null &&
        answer.context.prompts.length > 0
      ) {
        var dialogOptions = getDialogOptionsValue(stepContext);
  ...
  async displayQnAResult(stepContext) {
    ...
    if (responses != null) {
      if (responses.length > 0) {
        await stepContext.context.sendActivity(
          await outputTranslate(responses[0].answer)
        );
      } else {
        CurrentLanguage.swap();
        await stepContext.context.sendActivity(
          await outputTranslate(qnaDialogResponseOptions.noAnswer)
        );
      }
    }
    return await stepContext.endDialog();
...
//Async function calling request
async function outputTranslate(text) {
  let output;
  try {
